...
lucros = c(3, 6, 12, 8, 5, 10, 8, 4, 2, 9)
pesos  = c(3, 4, 11, 6, 9, 12, 10, 4, 5, 1)
capacidade = 20
...
problema_mochila <- function(tp, tc, ng) {
  ...
  cat("\n media: ", mean(fitness), "\t desvio-padrao:", sd(fitness), "\n \n")

  # Imprime a aptidão do melhor indivíduo, do pior e a média
  cat("PRIMEIRA POPULAÇÃO \t |", max(fitness), "\t |", mean(fitness), "\t |", min(fitness), "\n")
  ...
  for (i in 1:ng) {
    ...
    # Imprime a aptidão do melhor indivíduo, do pior e a média
    cat("GERAÇÃO DE FILHOS", i, "\t |", max(fitness), "\t |", mean(fitness), "\t |", min(fitness), "\n")
  }
